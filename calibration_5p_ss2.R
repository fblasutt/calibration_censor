...
text(1559,3.65,pos=4,"First Roman \n Index")
abline(v=1544,col=pc[2])
text(1544,3.65,pos=2,"First Index \n of the University \n of Paris")
...
mini<-function(betaa,pe,muu,thet,pe1,qu,qur){
  sum=0     
...
GA <- ga(type = "real-valued", 
         fitness = ff, 
         lower = c(0,20,0.1,0.05,0.05,Eq[1]-1,Eqr[1]-1),
         upper = c(0.7,950,8,0.4,950,Eq[1]+1,Eqr[1]+1),
...
if(serrors){
  ...
  for(j in 1:nboots){
    ...
    gammaB[j]<-GA@solution[1,5]
    SqB[j]<-GA@solution[1,6]
    SqrB[j]<-GA@solution[1,7]
    
    pB[j]=pB[j]^(-thetaB[j])
    
    gammaB[j]=pB[j]/( gammaB[j]^(-theta))
  ...
  cat(paste('\\begin{table}[htpb]
    ...
    Price of revolutionary books   & $p$   &',round(p^(-theta), digits=2),'& ',round(pS, digits=3),' \\\\[0.15ex]
     Self Censorship Parameter   & $\\gamma$   &',round(p^(-theta)/(pr1^(-theta)), digits=2),'& ',round(gammaS, digits=3),' \\\\[0.15ex]
    \\hline
    \\end{tabular}
    \\label{table:param}
    \\end{table}'))
