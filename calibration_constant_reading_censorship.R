...
impbeta <<-1    #imperfectSqzr application of censorship-max 1/.80
...
zt<-function(beta,pe,t,up,thet,eqct,eqrtt) {
  ...
  if(t==3){
    z0=pe/(1)*(z0*(1)/pe)^(2)
    (1-beta)/(pe+beta*z0)*(z0^2)
  }else if(t==4){
    z0=pe/(1)*(z0*(1)/pe)^(2)
    z1=(1-beta)/(pe+beta*z0)*(z0^2)
    (1-beta)/(pe+beta*z1)*(z1^2)
  }else if(t==5){
    z0=pe/(1)*(z0*(1)/pe)^(2)
    z1=(1-beta)/(pe+beta*z0)*(z0^2)
    z2=(1-beta)/(pe+beta*z1)*(z1^2)
    (1-beta)/(pe+beta*z2)*(z2^2)
  }else if(t==2){pe/(1)*(z0*(1)/pe)^(2^(t-1))
...
if (longe) {mu<-function(t) ifelse(t==1,68.18-18,ifelse(t==2,63.95-18,ifelse(t==3,65.36-18,ifelse(t==4,64.73-18,ifelse(t==5,69.73-18,0)))))/(68.18-18)}
...
qcs<-function(beta,pe,t,up,pre,nu,thet,eqct,eqrtt) if(t<=2){gamma(1-thet)*(((1+nu)*mu(t)*((pre/gamma(1-thet))^(1/thet)*
                                                                                           ...
                                                                                           (1-(1-beta)*mt(beta,pe,t-1,up,thet,eqct,eqrtt)))))^thet}             
...
nc<-function(beta,pe,t,up,pre1,pre2,nu,thet,eqct,eqrtt) if(t<=2){qrs(0,pe,t,up,pre1,nu,thet,eqct,eqrtt)*
    ...
    {qrs(beta,pe,t,up,pre1,nu,thet,eqct,eqrtt)*
        ((1-beta)*mt(beta,pe,t,up,thet,eqct,eqrtt))+
        ((1-(1-beta)*mt(beta,pe,t,up,thet,eqct,eqrtt)))*
...
GA <- ga(type = "real-valued", 
         fitness = ff, 
         lower = c(0.12,1.7,0.0,0.05,3.5-0.35,Eqr[1]-0.48),
         upper = c(0.25,2.5,3  ,0.45,3.5+0.4 ,Eqr[1]+0.0),
...
if(normal){results<-c((Sq[5]-SqC[5])/Sq[5]*100,(Sm[5]-SmC[5])/Sm[5]*100,beta*100,Sm[5])}
if(!normal){results<-c((Sq[5]-SqC[5])/Sq[5]*100,(Sm[5]-SmC[5])/Sm[5]*100,beta*100,Sm[5])}
