...
for(i in 1:length(t)){
  ...
  qual2[i]=mean(members$pubb[members$refdatel>=t[i]-lim/2 & members$refdatel<t[i]+lim/2])
...
text(1583,4.8 ,"First Roman \n Index",adj = c(0.5, NA))
abline(v=1544,col=pc[2])
text(1515,4.2,"First Index \n of the University \n of Paris",adj = c(0.5, NA))
...
qrs<-function(beta,pe,t,up,mu,pre,thet,eqtt,eqrtt) if(t<=2){gamma(1-thet)*(mu*((1)*(pre/gamma(1-thet))^(1/thet)*
                                                                                 (mt(0,pe,t-1,up,thet,eqtt,eqrtt))))^thet}else
                                                                                 {gamma(1-thet)*(mu*((1-beta)*(pre/gamma(1-thet))^(1/thet)*
                                                                                                       (mt(beta,pe,t-1,up,thet,eqtt,eqrtt))))^thet} 


qcs<-function(beta,pe,t,up,mu,pre,thet,eqtt,eqrtt) if(t<=2){gamma(1-thet)*((mu*((pre/gamma(1-thet))^(1/thet)*
                                                                                  (1-mt(0,pe,t-1,up,thet,eqtt,eqrtt)))))^thet}else
                                                                                  {gamma(1-thet)*((mu*((pre/gamma(1-thet))^(1/thet)*
                                                                                                         (1-mt(beta,pe,t-1,up,thet,eqtt,eqrtt)))))^thet}             


qs<-function(beta,pe,t,up,mu,pre1,pre2,thet,eqtt,eqrtt) if(t<=2){qrs(0,pe,t,up,mu,pre1,thet,eqtt,eqrtt)*mt(0,pe,t,up,thet,eqtt,eqrtt)+
    (1-mt(0,pe,t,up,thet,eqtt,eqrtt))*qcs(0,pe,t,up,mu,pre2,thet,eqtt,eqrtt)}else
    {qrs(beta,pe,t,up,mu,pre1,thet,eqtt,eqrtt)*mt(beta,pe,t,up,thet,eqtt,eqrtt)+
        (1-mt(beta,pe,t,up,thet,eqtt,eqrtt))*qcs(beta,pe,t,up,mu,pre2,thet,eqtt,eqrtt)}

nc<-function(beta,pe,t,up,mu,pre1,pre2,thet,eqtt,eqrtt) if(t<=2){qrs(0,pe,t,up,mu,pre1,thet,eqtt,eqrtt)*
    (((1)*mt(0,pe,t,up,thet,eqtt,eqrtt))/(1-mt(0,pe,t,up,thet,eqtt,eqrtt)))+
    ((1-mt(0,pe,t,up,thet,eqtt,eqrtt))/(1-mt(0,pe,t,up,thet,eqtt,eqrtt)))*
    qcs(0,pe,t,up,mu,pre2,thet,eqtt,eqrtt)}else
    {qrs(beta,pe,t,up,mu,pre1,thet,eqtt,eqrtt)*
...
mini<-function(betaa,pe,muu,thet,qu,qur){
  sum=0     
...
GA <- ga(type = "real-valued", 
         fitness = ff, 
         lower = c(0,20,0.1,0.05,Eq[1]-1,Eqr[1]-1),
         upper = c(0.7,850,8,0.4,Eq[1]+1,Eqr[1]+1),
         popSize = 100, maxiter = 80, run = 20,
...
###########################################################
#This part here for computing standard errors of parameters
############################################################
...
for(i in 1:5){
  ...
  Smbeta[i]=beta*mt(beta,p,i,max,theta,Sq[1],Sqr[1])
  ...
  if(i>=2){
    ...
    SqrM[1]=(((Sqr[1]/(gamma(1-theta)))^(1/1))/
               ((log(2))^theta))
    SNC[1]=Sqr[1]*
      (((1)*mt(beta,p,i,max,theta,Sq[1],Sqr[1]))/(1-mt(beta,p,i,max,theta,Sq[1],Sqr[1])))+
      ((1-mt(beta,p,i,max,theta,Sq[1],Sqr[1]))/(1-mt(beta,p,i,max,theta,Sq[1],Sqr[1])))*
      Sqc[1]
...
pdf(file="fit.pdf", width = 10.3, height = 5)
par(mfrow=c(2,3),oma = c(3, 0.5, 0.5, 0.5),
...
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q_t(50p)$'),cex.lab=1.3,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
polygon(c(time,rev(time)),c(CIq[1,],rev(CIq[2,])),col =pc[3], border = FALSE)
...
yrange <- c(3,9)
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q_t(75p)$'),cex.lab=1.3,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
polygon(c(time,rev(time)),c(CIq75[1,],rev(CIq75[2,])),col =pc[3], border = FALSE)
...
yrange <- c(3,9)
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q^R_t(50p)$'),cex.lab=1.3,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
yrange <- c(3,9)
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q^R_t(75p)$'),cex.lab=1.3,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$m_t\\beta_t$'),cex.lab=1.3,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX("$m_t\\beta_t$"),cex.lab=1.4,xaxt='n')
...
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$m_t$'),cex.lab=1.4,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
yrange <- c(2,9)
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q_t$'),cex.lab=1.4,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
yrange <- c(2,9)
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q^R_t$'),cex.lab=1.4,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
yrange <- c(2,9)
plot(xrange,yrange,type="n", xlab="Time",
     ylab=TeX('$q^C_t$'),cex.lab=1.4,xaxt='n')
axis(1, at=1:5,labels=timee,font=2)
...
legend("bottom", legend = c( "Simulation",TeX('$Countertfactual:\\beta_t=0$')), 
       col=pc, lwd=3, cex=2,horiz = TRUE, lty=c(1,2),xpd=TRUE,bty = 'n',inset = c(0.1,0))
dev.off()
...
print(((Sq[5]-(Sm[5]*SqrC[5]+(1-Sm[5])*SqcC[5]))/Sq[5]))


################################################
#BOOTSTRAPED STANDARD ERRORS FOR PARAMETERS
###############################################
serrors<-logical()
serrors<-FALSE
...
cat(paste('\\begin{table}[htpb]
    ...
    \\hline % inserts single horizontal line
    \\rule{0pt}{2.5ex}
    Mean quality in 1  & $\\overline{q}_1$   &',round(Sq[1], digits=2),'&',round(SqS, digits=3),' \\\\[0.15ex]
    Mean R quality in 1  & $\\overline{q}^R_1$   &',round(Sqr[1], digits=2),'&',round(SqrS[1], digits=3),' \\\\[0.15ex]
     Productivity of books  & $\\theta$   &',round(theta, digits=2),'& ',round(thetaS, digits=3),' \\\\[0.15ex]
    \\ Max Censorship  & $\\overline{\\beta}$   &',round(beta, digits=2),'& ',round(betaS, digits=3),'  \\\\[0.15ex]
    Knowledge growth & $(1+\\nu)\\mu$   &',round(mu, digits=2),'& ',round(muS, digits=3),'  \\\\[0.15ex]
    Price of revolutionary books   & $p$   &',round(p^(-theta), digits=2),'& ',round(pS, digits=3),' \\\\[0.15ex]
    \\hline
    \\end{tabular}
    \\label{table:param}
    \\end{table}'))
